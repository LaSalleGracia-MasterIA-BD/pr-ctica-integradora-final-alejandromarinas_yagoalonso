...
| T4 | `requirements-dashboard.txt` con `streamlit==1.36.0`, `httpx==0.27.0`, `plotly==5.22.0`, `pandas==2.2.2`. **SIN pillow** (`st.image` acepta bytes PNG directamente). Smoke local: `python -c "import streamlit, httpx, plotly, pandas; print(...)"` | RNF-1 | — | S | pending |
